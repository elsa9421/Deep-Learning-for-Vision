...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
        dists[i] = ((x_train[i].reshape(1, -1) - x_test.reshape(num_test, -1))**2).sum(dim = 1)
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  #dists = ((x_train.reshape(num_train, 1, -1) - x_test.reshape(num_test, -1))**2).sum(dim = 2)
  ## (x - y)^2 = x^2 + y^2 + 2.x.y
  dists = ((x_train**2).reshape(num_train, 1, -1)).sum(dim = 2) + \
          ((x_test**2).reshape(num_test, -1)).sum(dim = 1) - \
          2 * torch.matmul(x_train.reshape(num_train, -1), x_test.reshape(num_test, -1).t() )
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  #one_hot_encoder = torch.zeros([num_test, torch.unique(y_train)])
  for i in range(num_test):
          labels = y_train[torch.topk(dists[:,i], k=k, largest=False)[1]]
          class_no, count = torch.unique(labels, return_counts=True)
          y_pred[i] = class_no[count.max(dim=0)[1]]
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
        ...
        for i in range(num_folds):
                ...
                two_y = torch.Tensor([])
                if x_train_folds[:i]:
                      one = torch.cat(x_train_folds[:i])
                      one_y = torch.cat(y_train_folds[:i])
                if x_train_folds[i+1:]:
                      two = torch.cat(x_train_folds[i+1:])
                      two_y = torch.cat(y_train_folds[i+1:])
                Training_x = torch.cat((one,two))
                Training_y = torch.cat((one_y,two_y))
                classifier = KnnClassifier(Training_x, Training_y )
                lst.append(classifier.check_accuracy(torch.Tensor(x_train_folds[i]), 
                                  y_train_folds[i], k=k, quiet = True))
        k_to_accuracies[k] = lst
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k in k_to_accuracies.keys():
    mean_val = sum(k_to_accuracies[k])/len(k_to_accuracies[k])
    if mean_val > maxx:
        maxx = mean_val
        best_k = k
