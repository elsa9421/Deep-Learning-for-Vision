...
def mutate_tensor(x, indices, values):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  for (idx0, idx1),value in zip(indices,values):
    x[idx0, idx1]=value
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def multiples_of_ten(start, stop):
  ...
  if start % 10 == 0: 
    ...
    new_start = start + (10 - (start % 10))
  if new_start <= stop:
    x = torch.arange(new_start, stop+1, step = 10)
  else:
    x = torch.Tensor([])
...
def reshape_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  y = x.reshape(2,3,4).permute(1,0,2).reshape(3,-1)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  # Replace "pass" statement with your code
  y = x.clone()
  _, min_idxs = x.min(dim = 1)
  y[range(x.shape[0]), min_idxs] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def normalize_columns(x):
  ...
  M = x.shape[0]
  y = x.clone()
  y_mean = x.sum(dim = 0) / M
  y_std  = torch.sqrt( (1 / (M-1) ) * ((y - y_mean) ** 2).sum(dim = 0))
  y = (y - y_mean) / y_std
