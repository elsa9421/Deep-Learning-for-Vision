...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        enc = self.encoder(torch.cat((nn.Flatten()(x), c), axis = 1))
        mu, logvar = self.mu_layer(enc), self.logvar_layer(enc)
        z = reparametrize(mu, logvar)
        x_hat = self.decoder(torch.cat((z, c), axis = 1))
...
def reparametrize(mu, logvar):
    ...
    # std = sigma 
    # logvar = log(var)
    # logstd = 0.5log(var) => log(var^0.5) => log(std).exp() => std
...
def loss_function(x_hat, x, mu, logvar):
    ...
    KLD_loss = -(1/2)  * torch.sum(1 + logvar - mu**2 -  logvar.exp_())
    loss = (recon_loss + KLD_loss)/N
