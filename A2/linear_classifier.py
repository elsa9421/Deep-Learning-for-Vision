...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  # scores = X.mm(W)  #(N,D)(D,C)
  # correct_class_score = scores[range(0, X.shape[0]), y]  #(N,)
  # margin = scores - correct_class_score.reshape(-1,1) + 1   # (N,C)
  # dummy = torch.zeros_like(margin)
  # dummy[range(0, X.shape[0]), y] = margin[range(0, X.shape[0]), y]
  # count = margin - dummy
  # mask = (count > 0)
  # c = torch.sum(mask, dim =0)
  # margin[margin<=0] = 0
  # loss = torch.sum(torch.sum(margin, dim=1) - margin[range(0, X.shape[0]), y])/X.shape[0]
  # loss += reg * torch.sum(W * W)
  ...
  margin = scores - correct_class_score.reshape(-1,1) + 1   # (N,C)
  margin[margin<0] = 0
  margin[range(0, X.shape[0]), y] = 0  
  loss = torch.sum(margin)/X.shape[0]
  loss += reg * torch.sum(W * W)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  ...
  count = scores - correct_class_score.reshape(-1,1) + 1   # (N,C)
  count[count > 0] = 1
  count[count < 0] = 0
  count[range(0,X.shape[0]), y] = 0  # ignore count values where j == y[i]
  count[range(0,X.shape[0]), y] = -1 * torch.sum(count, dim=1)
  dW = torch.mm(X.t(),count)  #(D,N)(N,C) -> (D,C)

  # dW[:, range(0,10)] = c[range(0,10)].reshape(1,-1) * torch.sum(X, dim=0).reshape(-1, 1)
  # dW[:, y] = -c[y].reshape(1,-1) * torch.sum(X, dim=0).reshape(-1, 1)
  dW /= X.shape[0]
...
def svm_get_search_params():
  ...
  learning_rates = [0.001, 0.002 ,0.003, 0.004, 0.006, 0.01]
  regularization_strengths = [0.01, 0.02, 0.03]
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  train_acc = 100 * torch.sum(cls.predict(data_dict['X_train']) == data_dict['y_train']).item()\
                        /len(data_dict['X_train'])
  val_acc = 100 * torch.sum(cls.predict(data_dict['X_val']) == data_dict['y_val']).item()\
                        /len(data_dict['X_val'])
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(0, num_train):
    scores = W.t().mv(X[i])  #(C,D)(D,) -> (C,1)
    scores -= torch.max(scores)  # For numerical stability
    for j in range(0, num_classes):
      prob = torch.exp(scores[j]) / torch.sum(torch.exp(scores))
      dW[:, j]  += X[i] * prob.item()
      if j == y[i]:
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  loss = torch.sum(-torch.log(prob[range(0,num_train), y])).item() / num_train
  loss += reg * torch.sum(W * W)

  
  dW = torch.mm(X.t(), prob) #(D,N)(N,C)
  mat = torch.zeros_like(prob) # to account for j == y[i]
  mat[range(0,num_train), y] = 1
  dW -= torch.mm(X.t(), mat)  #Subtracting X[i] from dW when j == y[i]
  dW /= num_train
  dW += 2 * reg * W
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [0.04, 0.06, 0.08]
  regularization_strengths = [0.04, 0.035, 0.05]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
