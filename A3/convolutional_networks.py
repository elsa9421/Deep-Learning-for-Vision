...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    N, C, H, W = x.shape
    F, _, HH, WW = w.shape
    H_dash = 1 + (H + 2 * conv_param['pad'] - HH) // conv_param['stride']
    W_dash = 1 + (W + 2 * conv_param['pad'] - WW) // conv_param['stride']
    out = torch.empty((N, F, H_dash, W_dash), dtype=x.dtype, device=x.device)
    

    padded_x = torch.nn.functional.pad(x,
                      (conv_param['pad'], conv_param['pad'], conv_param['pad'], conv_param['pad']), 
                      mode='constant', value=0)


    for n in range(N):
        for f in range(F):
            for hh in range(0, H_dash):
              start_h = hh * conv_param['stride']
              end_h = start_h + HH
              for ww in range(0, W_dash):
                  start_w = ww * conv_param['stride']
                  end_w = start_w + WW
                  window = padded_x[n, :, start_h:end_h, start_w:end_w]
                  out[n, f, hh, ww] = torch.sum(window * w[f]) + b[f]
    ...
    for n in range(N):
        ...
        for f in range(F):
            for h_dash in range(H_dash):
                start_h = h_dash * conv_param['stride']
                end_h = start_h + HH
                for w_dash in range(W_dash):
                  start_w = w_dash * conv_param['stride']
                  end_w = start_w + WW
                  dw[f] += padded_x[n, :, start_h:end_h, start_w:end_w] * \
                           dout[n, f, h_dash, w_dash]
                  padded_dx[n, :, start_h:end_h, start_w:end_w] += w[f] * \
                                                dout[n, f, h_dash, w_dash]
    
    dx = padded_dx[:, :, conv_param['pad']:H+conv_param['pad'], conv_param['pad']:W+conv_param['pad']]
...
class MaxPool(object):
  ...
  def forward(x, pool_param):
    ...
    H_dash = 1 + (H - p_H) // pool_param['stride']
    W_dash = 1 + (W - p_W) // pool_param['stride']
    out = torch.empty((N, C, H_dash, W_dash), dtype=x.dtype, device=x.device)
    
    for n in range(N):
        for h_dash in range(H_dash):
            h_start = h_dash * pool_param['stride']
            h_end = h_start + p_H
            for w_dash in range(W_dash):
                w_start = w_dash * pool_param['stride']
                w_end = w_start + p_W
                window = x[n, :, h_start:h_end, w_start:w_end]
                out[n, :, h_dash, w_dash] = (window.reshape(C, -1)).max(dim =-1)[0] 
  ...
  def backward(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for h_dash in range(H_dash):
              h_start = h_dash * stride
              h_end = h_start + p_H
              for w_dash in range(W_dash):
                w_start = w_dash * stride
                w_end = w_start + p_W
                window = (x[n, c, h_start:h_end, w_start:w_end]).reshape(-1)
                wind_mask = torch.zeros_like(window)
                wind_mask[torch.argmax(window)] = 1           
                dx[n, c, h_start:h_end, w_start:w_end] += wind_mask.reshape(p_H, p_W) * dout[n, c, h_dash, w_dash]
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    ## For Hidden linear layer  --> output from max pool --(N,F,H,W)
    self.params['W2'] = torch.randn(num_filters*(H//2)*(W//2), hidden_dim,\
                                   dtype=dtype, device=device) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    dx3, grads['W3'], grads['b3'] = Linear.backward(dx, cache3)
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    for i in range(1, self.num_layers):
      ...
      if self.batchnorm:
        self.params['gamma{}'.format(i)] = torch.ones(F, dtype=dtype, device=device)
        self.params['beta{}'.format(i)] = torch.zeros(F, dtype=dtype, device=device)
      
      if weight_scale == "kaiming":        
        self.params['W{}'.format(i)] = kaiming_initializer(C, F, K=HH, relu=True,\
                                                          device=device, dtype=dtype)
      else:
        self.params['W{}'.format(i)] = (torch.randn(F, C, HH, WW,\
                                         dtype=dtype, device=device)) * weight_scale
      self.params['b{}'.format(i)] = torch.zeros(F, dtype = dtype, device=device,)
      
      C = F
    
    flatten = F * H * W // (4 ** (len(self.max_pools)))
    if weight_scale == "kaiming":      
      self.params['W{}'.format(i+1)] = kaiming_initializer(flatten, num_classes,\
                                                      K=None, relu=False,\
                                                     dtype=dtype, device=device)
    else:
      self.params['W{}'.format(i+1)] = torch.randn(flatten, num_classes,\
                                         dtype=dtype, device=device) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    for idx in range(1, self.num_layers + 1):
      W = 'W{}'.format(idx)
      b = 'b{}'.format(idx)
      gamma = 'gamma{}'.format(idx)
      beta = 'beta{}'.format(idx)
      cache_no = 'cache{}'.format(idx)
        ...
        if self.batchnorm:
          if (idx - 1) in self.max_pools:
            scores, cache = Conv_BatchNorm_ReLU_Pool.forward(scores,\
                     self.params[W], self.params[b], self.params[gamma],\
                     self.params[beta], conv_param, self.bn_params[idx - 1], pool_param)
          else:
            scores, cache = Conv_BatchNorm_ReLU.forward(scores,\
                                self.params[W], self.params[b], self.params[gamma],\
                                self.params[beta], conv_param, self.bn_params[idx - 1])
        else:
          if (idx - 1) in self.max_pools:
            scores, cache = Conv_ReLU_Pool.forward(scores, self.params[W],\
                                             self.params[b], conv_param, pool_param)
          else:
            scores, cache = Conv_ReLU.forward(scores, self.params[W],\
                                                   self.params[b], conv_param)

      self.cache[cache_no] = cache
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for idx in range(self.num_layers, 0, -1):
      ...
      if idx == self.num_layers:
        dx, grads[W], grads[b] = Linear.backward(dx, self.cache[cache_no])
        
      else:          
        if self.batchnorm:
          if (idx - 1) in self.max_pools:
            dx, grads[W], grads[b], grads[gamma], grads[beta] =\
                                  Conv_BatchNorm_ReLU_Pool.backward(dx, self.cache[cache_no])
          else:
            dx, grads[W], grads[b], grads[gamma], grads[beta] =\
                                  Conv_BatchNorm_ReLU.backward(dx, self.cache[cache_no])
        else:
          if (idx - 1) in self.max_pools:
            dx, grads[W], grads[b] = Conv_ReLU_Pool.backward(dx, self.cache[cache_no])
          else:
            dx, grads[W], grads[b] = Conv_ReLU.backward(dx, self.cache[cache_no])
...
def find_overfit_parameters():
  ...
  ############################################################################
  # Replace "pass" statement with your code
  weight_scale = 3   # Experiment with this!
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  model = DeepConvNet(input_dims=input_dims,num_classes=10,
                      ...
                      max_pools=[0, 1, 2, 3],
                      reg=6e-5, weight_scale=weight_scale,
                      dtype=dtype, device=device)
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
  ...
  if K is None:
    ...
    # Replace "pass" statement with your code
    if K >= 0:
      weight_scale = torch.sqrt(torch.tensor(gain / (Din * K * K), dtype=dtype,\
                                device=device))
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      x_mu = x - sample_mean
      x_mu_sq = x_mu ** 2
      sample_var = torch.sum(x_mu_sq, dim = 0) / N
      denom = torch.sqrt(sample_var + eps)
      norm_factor = 1 / denom
      x_norm = x_mu * norm_factor

      
      running_mean = momentum * running_mean + (1 - momentum) * sample_mean
      running_var = momentum * running_var + (1 - momentum) * sample_var
      
      out = (gamma * x_norm + beta).to(x.dtype)
      cache = (x_norm, gamma, x_mu, norm_factor, denom, sample_var, eps, x, mode)
      ...
      out = (gamma * x_norm + beta).to(x.device)

      cache = (x_h, gamma, x, norm_factor, denom, running_var, eps, x_norm, mode)
  ...
  def backward(dout, cache):
    ...
    #   denom = torch.sqrt(sample_var + eps)
    #   norm_factor = 1 / den
    #   x_norm = x_new * norm_factor
    #   out = (gamma * x_normalized + beta).to(x.dtype)

    #   cache =  (x_norm, norm_factor, denom, sample_var, x_mu_sq, x_mu, x, gamma, eps, mode)
     
    N, D = dout.shape
    x_norm, gamma, x_mu, norm_factor, denom, sample_var, eps, x, mode = cache
    if mode == 'train':
      dbeta = torch.sum(dout, dim = 0)
      dgamma = torch.sum(dout * x_norm, dim = 0)
      dx_norm = (dout * gamma).to(dout.dtype)
      dr = torch.sum(dx_norm * x_mu, dim = 0)
      dx_1 = dx_norm * norm_factor
      dsv = -1 / (denom ** 2) * dr
      dv = 0.5 * 1. / torch.sqrt(sample_var + eps) * dsv
      d_s = (1 / N) *(torch.ones((N, D), dtype=dout.dtype, device=dout.device) * dv)
      dx_2 = (2 * x_mu * d_s).to(device = dout.device)
      dx_3 = dx_1 + dx_2
      d_m = -1 * torch.sum(dx_3, dim = 0)
      dx_4 = (1 / N)* (torch.ones((N, D), dtype=dout.dtype, device=dout.device) * d_m)
      dx = (dx_3 + dx_4).to(device=dout.device)

    else:
      dbeta = torch.sum(dout, dim = 0)
      dgamma = torch.sum(dout*x, dim = 0)
      dxh = (dout * gamma).to(dout.dtype)
      dr = torch.sum(dxh * x, dim = 0)
      dx = (dxh * norm_factor).to(device=dout.device)
  ...
  def backward_alt(dout, cache):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    (x_hat, gamma, x_new, _, _, var, eps, _, _) = cache
    N, D = dout.shape
    dx = gamma * (var + eps) ** -0.5 * (N * dout - torch.sum(dout, dim=0) -\
             x_new * (var + eps) ** (-1) * torch.sum(dout * x_new, dim = 0)) / N  
...
class SpatialBatchNorm(object):
  ...
  def backward(dout, cache):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = dout.shape
    dx, dgamma, dbeta = BatchNorm.backward((dout.permute(0, 2, 3, 1)).reshape(-1, C),\
                                             cache)
