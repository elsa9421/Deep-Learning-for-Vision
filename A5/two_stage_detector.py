...
class ProposalModule(nn.Module):
  def __init__(self, in_dim, hidden_dim=256, num_anchors=9, drop_ratio=0.3):
    ...
    self.pred_layer = nn.Sequential(nn.Conv2d(in_channels=in_dim,\
                                              out_channels=hidden_dim,\
                                              kernel_size=3, padding=1),
                                    nn.Dropout(drop_ratio),
                                    nn.LeakyReLU(),
                                    nn.Conv2d(in_channels=hidden_dim,\
                                              out_channels=(6*self.num_anchors),\
                                              kernel_size=1)
                                   )
    ##############################################################################
    #                               END OF YOUR CODE                             #
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    A = self.num_anchors
    if mode == 'train':
      pred = pred.reshape(B, A, -1, H, W) #BxAx6x7x7
      class_score = pred[:, :, :2 :, :]   #BxAx2x7x7
      anchor_data = pred[:, :, 2:, :, :]  #BxAx4x7x7
      anchor_idx = torch.cat((pos_anchor_idx, neg_anchor_idx))
      conf_scores = self._extract_anchor_data(class_score, anchor_idx) 
      offsets =  self._extract_anchor_data(anchor_data, pos_anchor_idx) 
      
      # input to GenerateProposal is of the form BxAxHxWx4 ->1x1x1xMx4 

      
      proposals = GenerateProposal(pos_anchor_coord.unsqueeze(0).unsqueeze(0).unsqueeze(0),\
                                   offsets.unsqueeze(0).unsqueeze(0).unsqueeze(0),\
                                   method='FasterRCNN')
      
    if mode == 'eval':
      pred = pred.reshape(B, A, -1, H, W) #BxAx6x7x7
      conf_scores = pred[:, :, :2 :, :]   #BxAx2x7x7
      offsets = pred[:, :, 2:, :, :]  #BxAx4x7x7
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    pos_anchor_idx, neg_anc_idx, GT_conf_scores, GT_offsets, GT_class, activated_anc_coord , _ =\
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    with torch.no_grad():
      ...
      conf_scores = conf_scores[:, :, 0, :, :].reshape(B, -1)
      sig_conf_scores = torch.sigmoid(conf_scores)

      proposals = GenerateProposal(anc_list, offsets.permute(0, 1, 3, 4, 2), method='FasterRCNN')
      proposals = proposals.reshape(B, -1, anc_list.shape[-1]) # (B, A, H, W, 4) -> # (B, -1, 4)
      
      # A = offsets.shape[1]
      # offsets = offsets.permute(0, 1, 3, 4, 2)  #(B, A, 4, H, W) ->(B, A, H, W, 4)
      # offsets = offsets.reshape(B, -1, A)


      for i in range(B):
        # HINT: Threshold the conf_scores based on the threshold value `thresh`.     #
        # Then, apply NMS to the filtered proposals given the threshold `nms_thresh`
        conf_scores_batch = sig_conf_scores[i, :] 
        ...
        final_conf_probs.append(scores[keep].unsqueeze(1))
...
class TwoStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    proposals_roi = torch.zeros((proposals.shape[0], 5), device=bboxes.device, dtype=bboxes.dtype)
    
    batch_idx = pos_anchor_idx // anc_per_img
    proposals_roi[:, 0] = batch_idx
    proposals_roi[:, 1:] =  proposals
    output_roi = torchvision.ops.roi_align(features, proposals_roi, output_size=(self.roi_output_w, self.roi_output_h))
    #output_roi.shape = (num_pos_proposals, C, 2, 2)
    
    # meanpool the feature in the spatial #
    #     dimension. 
    roi_feature =  nn.AvgPool2d((self.roi_output_h, self.roi_output_w)) (output_roi)
    roi_feature = roi_feature.squeeze()  # #(num_pos_proposals, C, 1, 1) ->(num_pos_proposals, C)
  

     # iii) Pass the RoI feature through the region classification layer which    #
    #      gives the class probilities. 
    class_prob = self.cls_layer(roi_feature)
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    with torch.no_grad():
      ...
      output_roi = torchvision.ops.roi_align(features, final_proposals, output_size=(self.roi_output_w, self.roi_output_h))
      roi_feature =  nn.AvgPool2d((self.roi_output_h, self.roi_output_w)) (output_roi)
      roi_feature = roi_feature.squeeze()  # #(num_pos_proposals, C, 1, 1) ->(num_pos_proposals, C)
      ...
      start = 0
      for i in range(N):
         end = start + final_proposals[i].shape[0]  # ( gives number of proposals in each image)
         final_class.append (class_prob[start : end].unsqueeze(1))
         start = end
