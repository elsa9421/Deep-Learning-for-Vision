...
def three_layer_convnet(x, params):
  ...
  conv_layer1 = F.relu(F.conv2d(x,  conv_w1, bias=conv_b1, stride=1, padding=2))
  conv_layer2 = F.relu(F.conv2d(conv_layer1,  conv_w2, bias=conv_b2, stride=1, padding=1))
  x_flatten = flatten(conv_layer2)
  scores = F.linear(x_flatten, fc_w, fc_b)
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.net = nn.Sequential(nn.BatchNorm2d(Cin),
                             ...
                             nn.BatchNorm2d(Cout),
                             nn.ReLU(),
                             nn.Conv2d(Cout, Cout, kernel_size=3, padding=1, stride=1),
                             )
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    Cout_stem = stage_args[0][0]
    modules = []
    modules.append(ResNetStem(Cin, Cout_stem))
    for i in range(len(stage_args)):
      modules.append(ResNetStage(*stage_args[i], block=block))
  ...
  def forward(self, x):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    resnet_out = self.cnn(x)
    avg_pool = F.avg_pool2d(resnet_out, resnet_out.shape[-1],resnet_out.shape[-1])
    scores = self.fc(flatten(avg_pool))
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
    return scores
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(nn.BatchNorm2d(Cin),
                             nn.ReLU(),
                             nn.Conv2d(Cin, Cout // 4, kernel_size=1, stride=s),
                             nn.BatchNorm2d(Cout // 4),
                             nn.ReLU(),
                             nn.Conv2d(Cout // 4, Cout // 4, kernel_size=3, padding=1, stride=1),
                             nn.BatchNorm2d(Cout // 4),
                             nn.ReLU(),
