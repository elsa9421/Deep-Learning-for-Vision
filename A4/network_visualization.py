...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for i in range(max_iter):
    output = model(X_adv)
    X_adv_score, idx  = output.max(dim=1)
    if verbose:
      print("Iter={}/{} target score={:.4f} score of X_adv={:.4f}"\
            .format(i, max_iter, output[:, target_y].item() , X_adv_score.item())) 
    if idx == target_y:
       break
    loss = output[:, target_y]
    loss.backward()
    g = X_adv.grad.data
    with torch.no_grad():
      X_adv += learning_rate  * g / torch.norm(g)
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    g = img.grad.data
    with torch.no_grad():
       img += learning_rate *  ( g / torch.norm(g) - 2 * l2_reg * img )
    img.grad.data.zero_()
