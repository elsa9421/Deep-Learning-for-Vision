...
def content_loss(content_weight, content_current, content_original):
    ...
    return content_weight * torch.nn.functional.mse_loss(content_current, content_original, reduction='sum') 
...
def gram_matrix(features, normalize=True):
    ...
    gram = torch.matmul(F, F.transpose(2, 1)) #(N, C, H*W) (N, H*W, C)
    if normalize:
      gram = gram * (1 / (H * W * C))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx, i in enumerate(style_layers):
      ...
      style_loss += style_weights[idx] * nn.functional.mse_loss(G, style_targets[idx], reduction='sum')
    return style_loss
...
def tv_loss(img, tv_weight):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = img.shape
    horiz = nn.functional.mse_loss(img[:, :, :-1, :], img[:, :, 1:, :], reduction = 'sum')
    vert =  nn.functional.mse_loss(img[:, :, :, :-1], img[:, :, :, 1:], reduction = 'sum')
    tv_loss = tv_weight * (horiz + vert)
