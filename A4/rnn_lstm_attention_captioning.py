...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    h_t = torch.mm(prev_h, Wh) + torch.mm(x, Wx) + b
    next_h = torch.tanh(h_t)  # (N, H)
    cache = (h_t, Wx, Wh, prev_h, x)
...
def rnn_step_backward(dnext_h, cache):
    ...
    h_t, Wx, Wh, prev_h, x  = cache
    local_grad =  (1 - (torch.tanh(h_t))** 2 ) * dnext_h   #(N, H)
    dx = torch.mm(local_grad, Wx.transpose(0, 1))   #(N, H) (H, D) ->(N, D)
    dprev_h = torch.mm(local_grad, Wh.transpose(0, 1))           #(N, H)*(H,H)-->(N, H)
    dWx =  torch.mm(x.transpose(0, 1), local_grad)   #(D, N)(N, H)*#(D, H)
    dWh = torch.mm(prev_h.transpose(0, 1), local_grad)  #(N, H)(N, H)
    db =  torch.sum(local_grad, dim=0)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      if t == 0:
        ...
        h[:, t, :], step_cache[t] = rnn_step_forward(x[:, t, :], h[:, t-1, :].clone(), Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      dh_time = dh[:, t, :] + dprev_h
      dx[:, t, :], dprev_h, dWx_step, dWh_step, db_step = rnn_step_backward(dh_time, step_cache[t])
      dWx += dWx_step
      dWh += dWh_step
      db += db_step
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    # Replace "pass" statement with your code
    loss = 0
    for batch_no in range(x.shape[0]):
      loss += (F.cross_entropy(x[batch_no], y[batch_no], reduction= 'sum', ignore_index=ignore_index))
    loss /= x.shape[0]
    # idx = list(range(x.shape[0]))
    # loss = (F.cross_entropy(x[idx, : , :], y[idx, :], reduction= 'sum', ignore_index=ignore_index)) / x.shape[0] 
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        if self.cell_type == 'rnn':
          ...
          self.model_type = RNN(wordvec_dim, hidden_dim, device=device, dtype=dtype)
        elif self.cell_type == 'lstm':
          self.feature_extract = FeatureExtractor(pooling=True, dtype=dtype, device=device)
          self.model_type = LSTM(wordvec_dim, hidden_dim, device=device, dtype=dtype)
        elif self.cell_type == 'attention':
          self.feature_extract = FeatureExtractor(pooling=False, dtype=dtype, device=device)
          self.model_type = AttentionLSTM(wordvec_dim, hidden_dim, dtype=dtype, device=device)

        self.affine = nn.Linear(input_dim, hidden_dim).to(device=device, dtype=dtype)  #(N, H)
    ...
    def forward(self, images, captions):
        ...
        ############################################################################
        # Replace "pass" statement with your code
        feat = self.feature_extract.extract_mobilenet_feature(images)   #(N, 1280)
        if self.cell_type == 'rnn' or self.cell_type == 'lstm': 
           h0 = self.affine(feat)
        elif self.cell_type == 'attention':
           h0 = self.affine(feat.permute(0,2,3,1)).permute(0,3,1,2)

        layer2 = self.wordembed(captions_in)   #output -> (N, T-1, W)
        layer3 = self.model_type(layer2, h0)   #output -> (N, T-1, H)
        
        layer4 = self.temporal_affine(layer3)  # (N, T, V)


        loss = temporal_softmax_loss(layer4, captions_out, ignore_index=self._null)
    ...
    def sample(self, images, max_length=15):
        ...
        #   next_h =  self.model_type.step_forward(embed_word, prev_h)
        #   scores = self.temporal_affine(next_h)
        #   _, highest_scores = scores.max(dim=1)
        #   captions[:, i] = highest_scores
        #   for n in range(N):
        #     prev_word = highest_scores[n].item()
        #     embed_word[n] = self.wordembed(prev_word)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b, attn=None, Wattn=None):
    ...
    _, D = x.shape
    N, H = prev_h.shape
    a = torch.mm(x, Wx) + torch.mm(prev_h, Wh) + b #(N, D)(D, 4H)
    if attn is not None:
      a += torch.mm(attn, Wattn)
    i = torch.sigmoid(a[:, :H])
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    h = torch.zeros((N, T, H), dtype=x.dtype, device=x.device)
    for t in range(T):
      h[:, t, :], prev_c = lstm_step_forward(x[:, t, :], prev_h, prev_c, Wx, Wh, b)
      prev_h = h[:, t, :].clone()
...
def dot_product_attention(prev_h, A):
    ...
    A_til = A.reshape(N, H, -1)
    M_til = (torch.matmul(A_til.permute(0, 2, 1), prev_h.unsqueeze(2))).squeeze()  #(N, 16)
    attn_weights = torch.softmax((M_til * (1 / math.sqrt(H))), dim = 1)   
    attn = torch.matmul(A_til, attn_weights.unsqueeze(2)).squeeze()    #(N, H , 16) (N, 16, 1)
...
def attention_forward(x, A, Wx, Wh, Wattn, b):
    ...
    for t in range(T):
      ...
      h[:, t, :], prev_c = lstm_step_forward(x[:, t, :], prev_h, prev_c, Wx, Wh, b, attn=attn, Wattn=Wattn)
      prev_h = h[:, t, :].clone()
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
